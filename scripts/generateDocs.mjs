...
/**
 * Replace all instances of @aurodesignsystem, slideshow, Slideshow, auro and Auro accordingly
 */
...
function formatTemplateFileContents(content, destination) {
  ...
  result = result.replace(/\slideshow(?!\()/g, nameExtractionData.name);
  result = result.replace(/\Slideshow(?!\()/g, nameExtractionData.nameCap);
